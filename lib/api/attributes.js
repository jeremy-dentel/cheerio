...
var setAttr = function(el, name, value) {
  if (typeof name === 'object') return $.extend(el.attribs, name);
...
var hasClass = exports.hasClass = function(className) {
  return $.any(this, function(elem) {
    var attrs = elem.attribs;
    return attrs && $.includes((attrs['class'] || '').split(rspace), className);
  });
};
...
var addClass = exports.addClass = function(value) {
  ...
  if (!value || !$.isString(value)) return this;
...
var removeClass = exports.removeClass = function(value) {
  ...
  return this.each(function(i, el) {
    if (!isTag(el)) return;
    el.attribs['class'] = (!value) ? '' : $.reject(
      split(el.attribs['class']),
      function(name) { return $.includes(classes, name); }
    ).join(' ');
  });
};
